...
export async function mouthCuesToInputFile({mouthCues, outputPath = '../ffmpeg/input.txt'} : {mouthCues: MouthCue[], outputPath?:string}){
    try {
...
export function generateMouthData(mouthCues : MouthCue[]) {
    ...
    for (const mouthCuesKey in mouthCues) {
        ...
        let tempStr = `file ../img/${mouthCues[mouthCuesKey].value}.png\noutpoint ${dur}\n`

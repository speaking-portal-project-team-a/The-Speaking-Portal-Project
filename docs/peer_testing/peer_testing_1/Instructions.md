...
*Ask user to delete the output json.*

### Missing File Test
Examiner: Please set `language_selection = 9` in `index.txt`. When you are ready, 
navigate in the terminal to `..\The-Speaking-Portal-Project\speaking-portal-project` and enter `npm start` 
(*Be sure to save `index.txt`*. *When user is ready have them view output mp4, then have them delete output json*)
...
`npm start`. (*Be sure to save `index.txt`*)
...
`language_selection = 5|6|7|8` (choose one integer to map respectivley to the extra files), and move the corresponding audio file into the rhubarb
